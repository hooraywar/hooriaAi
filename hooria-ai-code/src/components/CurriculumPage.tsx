...
async function fetchCurriculum(slug: string): Promise<CurriculumRow | null> {
  const { data, error } = await supabase
    .from("curriculums")
    .select(
      "id, title, subtitle, description, duration, prerequisites, class_duration, qa_session",
    )
    .eq("slug", slug)
...
export function CurriculumPage({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    if (curriculum) document.title = `${curriculum.title} — Hooria AI`;
  }, [curriculum]);
  ...
  const snapshotItems = [
    ...
    curriculum.class_duration && {
      ...
      value: curriculum.class_duration,
    },
    ...
    curriculum.qa_session && {
      icon: MessagesSquare,
      label: "Q&A Session",
      value: curriculum.qa_session,
    },
  ].filter((x): x is { icon: typeof Clock; label: string; value: string } =>
    Boolean(x),
  );
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          <Reveal className="text-center max-w-2xl mx-auto">
            ...
            <p className="mt-4 text-muted-foreground">
              {modules.length > 0
                ? `${modules.length} module${modules.length === 1 ? "" : "s"}. Every session is a live build. Every module ends with a shippable artifact.`
      ...
      <section className="py-20 sm:py-24 border-t border-border/40 bg-surface/30">
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          <Reveal className="text-center max-w-2xl mx-auto">
            ...
            <p className="mt-4 text-muted-foreground">
              You'll leave with a production-grade portfolio that proves you can
              build modern AI applications.
            </p>
          </Reveal>
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6 text-center">
          <Reveal>
            ...
            <p className="mt-6 text-lg text-muted-foreground max-w-2xl mx-auto">
              Don't just prompt — engineer. Join the next cohort and master the
